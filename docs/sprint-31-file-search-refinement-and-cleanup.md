...
- Tune search ranking/result quality within the existing BM25-first model
...
- Current BM25-first search path from Sprint 29

## Investigation Summary
- Sprints 27–30 delivered the separate file-search DB, scanner/indexer MVP, BM25-first search MVP, and scheduler-driven freshness/hardening MVP.
...
- Feature refinement is justified only in limited form:
  ...
  - ranking/snippet/result shaping can be improved within the existing BM25-first contract,
...
- AC-8: Search result quality is improved within the BM25-first contract through constrained ranking/snippet/result-shaping tuning, with the specific quality target defined by test coverage.
...
- [ ] **0.6** Add failing tests for ranking-quality expectations and chunking-policy behavior
  - Role: test-engineer
  - Deliverable: RED tests covering representative BM25-first/hybrid ranking expectations and stable chunk-boundary behavior
  - Depends on: 0.5
...
- [ ] **2.2** Tune ranking policy for BM25-first plus grounded semantic blending
  - Role: backend-coder
  - Deliverable: ranking logic that preserves BM25-first behavior while improving result quality when grounded semantic evidence exists
  - Depends on: 0.6, 2.1
...
- [ ] **3.2** Add end-to-end integration verification across BM25-only, gated-semantic-off, and grounded-semantic-on retrieval cases
  - Role: test-engineer
...
- Run ranking/chunking tests proving result quality improvements without breaking BM25-first correctness or index stability.
- Run end-to-end retrieval tests for BM25-only, semantic-gated-off, and semantic-gated-on paths.
...
- **Risk:** ranking/chunking tuning destabilizes the existing BM25-first path.
  **Mitigation:** preserve BM25-first as the default contract and require regression coverage for current indexed/search behavior.
...
- [ ] Ranking/chunking improvements are test-covered and preserve the BM25-first contract.
