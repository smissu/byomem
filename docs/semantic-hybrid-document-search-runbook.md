...
The scanner ignores legacy memory artifacts in project trees, including `native-store.json` and `native-store.json.migrated`, so file-search indexing does not depend on the memory snapshot files at all.
