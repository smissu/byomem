...
describe('runtime cli', () => {
  ...
  it('prints JSON usage for --help', async () => {
    ...
    await main(['--help']);
    expect(JSON.parse(String(spy.mock.calls.at(-1)?.[0] ?? '{}'))).toMatchObject({ error: 'Usage', commands: expect.arrayContaining(['store', 'search', 'connect codex', 'remove codex', 'file-search-scan', 'prune', 'generate', 'status', 'doctor', 'dashboard']) });
  });
