...
describe('search index baseline', () => {
  it('filters by scope and preserves deterministic ordering', async () => {
    const results = await searchIndex(createStore(), { query: 'alpha lexical only', scope: 'project', mode: 'hybrid' });
  ...
  it('returns hybrid results with lexical coverage preserved', async () => {
    const results = await searchIndex(createStore(), { query: 'alpha semantic rerank', mode: 'hybrid' });
