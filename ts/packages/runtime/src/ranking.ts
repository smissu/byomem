...
import { semanticFingerprint, semanticScore as groundedSemanticScore } from './semantic.js';
...
function totalScore(signals: SearchScore['signals'], mode: SearchMode): number {
  ...
  return (signals.lexical * 0.4) + (signals.semantic * 0.5) + (signals.scope * 0.05) + (signals.provenance * 0.03) + (signals.context * 0.015) + (signals.recency * 0.005);
}
...
export function rankRecord(record: MemoryRecord, query: string, mode: SearchMode = 'hybrid'): SearchScore {
  const signals = {
    ...
    semantic: groundedSemanticScore(record, query),
    scope: scopeScore(record),
    provenance: provenanceScore(record),
    context: contextScore(record, query),
    recency: recencyScore(record),
  };
  const score = totalScore(signals, mode) + (mode === 'hybrid' && signals.semantic > 0 ? 0.35 : 0);
  return { record, score, signals };
}
...
export function rankRecords(records: MemoryRecord[], query: string, mode: SearchMode = 'hybrid'): SearchScore[] {
  ...
  return [...bestById.values()].sort((a, b) => {
    if (mode === 'hybrid') {
      const semanticDelta = b.signals.semantic - a.signals.semantic;
